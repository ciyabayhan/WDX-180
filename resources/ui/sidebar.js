...
function findTypeRepeat(resources, types) {
  ...
  types.forEach((type) => {
    ...
    const entry = { type: type, count: counter, checked: true };
    entries.push(entry);
  });
  return entries;
}
...
function checkResult(resources) {
  document.querySelector(".job-wrapper").addEventListener("click", (event) => {
    if (event.target.id.includes("resource")) {
      const checkboxes = document.querySelectorAll(".job-wrapper input");
      const checkedResult = {};
      checkboxes.forEach((checkbox) => {
        const labelElement = document.querySelector(
          `.type-container label[for='${checkbox.id}']`,
        );
        const checkedText = labelElement.textContent;
...
function searchButton(data) {
  const inputElement = document.querySelector(".alert input");
  const buttonElement = document.querySelector(".alert button");
